...
/**
 * Trail effect - follows a moving object
 */
...
/**
 * Lightning/electricity particles
 */
export function ParticleLightning({
  position = [0, 0, 0],
  count = 30,
  color = '#ffeb3b',
  onComplete,
...
/**
 * Ice/freeze particles
 */
export function ParticleIce({
  position = [0, 0, 0],
  count = 50,
  color = '#4fc3f7',
  radius = 2,
  ...
  return (
    <ParticleSystem
      ...
      config={{
        ...
        spawnRadius: 0.5,
        velocity: [0, 0.2, 0],
        velocitySpread: [0.5, 0.2, 0.5],
        radialVelocity: radius,
        gravity: [0, -0.5, 0],
        sizeStart: 0.08,
        sizeEnd: 0.04,
        colorStart: '#ffffff',
...
// ============================================================================
// COMPOSITE EFFECTS (combining multiple particle systems)
// ============================================================================

/**
 * Card use effect - combines burst, ring, and sparkles
 */
export function CardUseEffect({
  position = [0, 0.5, 0],
  color = '#88c0d0',
  onComplete,
}) {
  const [phase, setPhase] = useState(0);
  const completedRef = useRef({ burst: false, ring1: false, ring2: false });

  const checkComplete = (key) => {
    completedRef.current[key] = true;
    if (Object.values(completedRef.current).every(Boolean)) {
      onComplete?.();
    }
  };
  ...
  return (
    <group position={position}>
      ...
      <ParticleBurst
        ...
        speed={4}
        lifetime={0.6}
        onComplete={() => checkComplete('burst')}
      />
      ...
      <ParticleRing
        ...
        expandSpeed={3}
        lifetime={0.7}
        onComplete={() => checkComplete('ring1')}
      />
      ...
      <ParticleRing
        ...
        expandSpeed={2.5}
        lifetime={0.8}
        onComplete={() => checkComplete('ring2')}
      />
    </group>
  );
}
...
/**
 * Ascension effect - dramatic rising particles
 */
export function AscensionEffect({ onComplete }) {
  const [progress, setProgress] = useState(0);
  
  useFrame((_, delta) => {
    setProgress(p => {
      const next = p + delta / 2.5;
      if (next >= 1) {
        onComplete?.();
        return 1;
      }
